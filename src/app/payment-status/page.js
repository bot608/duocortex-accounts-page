  ...
  const handlePaymentVerification = async (orderId, paymentId, amount) => {
    try {
      ...
      if (verification.status === 'SUCCESS') {
        ...
        const validAmount = amount && amount !== 'N/A' && !isNaN(amount) ? amount : null;
        setMessage(validAmount 
        ...
        throw new Error(`Payment verification failed: ${verification.payment_status || 'Unknown status'}`);
      }
    } catch (error) {
      console.error('Payment verification error:', error);
      setStatus('error');
      setMessage(`Payment verification failed: ${error.message}. If money was deducted, it will be refunded within 5-7 business days.`);
    }
  };
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-green-800">{paymentData.orderId}</span>
                    </div>
                    {paymentData.paymentId && (
                      <div className="flex justify-between">
                        <span className="text-green-700">Payment ID:</span>
                        <span className="font-mono text-green-800">{paymentData.paymentId}</span>
                      </div>
                    )}
                    {paymentData.amount && (
                      <div className="flex justify-between">
                        <span className="text-green-700">Amount:</span>
                        <span className="font-semibold text-green-800">₹{paymentData.amount}</span>
                      </div>
                    )}
                    {user && (
                      <div className="flex justify-between">
                        <span className="text-green-700">New Balance:</span>
                        <span className="font-semibold text-green-800">₹{user.coins || 0}</span>
                      </div>
                    )}
                  </div>
                </div>
